...
void listerplacesvoituresdisponibles(MYSQL *con) {
    ...
    if (mysql_query(con, "SELECT code,type FROM places WHERE type='VOITURE' AND etat=0")) {
        fprintf(stderr, "%s\n", mysql_error(con));
        mysql_close(con);
        exit(1);
    }
...
void listerplacescamionsdisponibles(MYSQL *con) {
    ...
    if (mysql_query(con, "SELECT code,type FROM places WHERE type='CAMIONS' AND etat=0")) {
        fprintf(stderr, "%s\n", mysql_error(con));
        mysql_close(con);
        exit(1);
    }
...
void listerplacesdeuxrouesdisponibles(MYSQL *con) {
    ...
    if (mysql_query(con, "SELECT code,type FROM places WHERE type='DEUX_ROUES' AND etat=0")) {
        fprintf(stderr, "%s\n", mysql_error(con));
        mysql_close(con);
        exit(1);
    }
...
void listerplacesoccupees(MYSQL *con) {
    ...
    if (mysql_query(con, "SELECT code,type FROM places WHERE etat=1")) {
        fprintf(stderr, "%s\n", mysql_error(con));
        mysql_close(con);
        exit(1);
    }
